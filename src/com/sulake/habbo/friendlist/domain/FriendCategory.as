package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategory implements IDisposable
   {
      ...
      public static const const_454:int = 100;
      ...
      {
         var _loc4_:* = null;
         if(this.var_260.indexOf(param1) >= 0)
         {
            return;
         }
         var _loc2_:String = param1.name.toLowerCase();
         var _loc3_:int = 0;
         while(_loc3_ < this.var_260.length)
         {
            _loc4_ = this.var_260[_loc3_];
            if(_loc2_ < _loc4_.name.toLowerCase())
            {
               this.var_260.splice(_loc3_,0,param1);
               return;
            }
            _loc3_++;
         }
      ...
      {
         var _loc2_:* = null;
         for each(_loc2_ in this.var_260)
         {
            if(_loc2_.selected)
            {
               param1.push(_loc2_);
      ...
      public function removeFriend(param1:int) : Friend
      {
         ...
         if(_loc2_ != null)
         {
            Util.remove(this.var_260,_loc2_);
            return _loc2_;
         }
         return null;
      }
      ...
      {
         if(this.var_1075 >= this.getPageCount())
         {
            this.var_1075 = Math.max(0,this.getPageCount() - 1);
         }
      }
      
      public function getPageCount() : int
      {
         return Math.ceil(this.var_260.length / const_454);
      }
      
      public function getStartFriendIndex() : int
      {
         this.checkPageIndex();
         return this.var_1075 * const_454;
      }
      
      public function getEndFriendIndex() : int
      {
         this.checkPageIndex();
         return Math.min((this.var_1075 + 1) * const_454,this.var_260.length);
      }
