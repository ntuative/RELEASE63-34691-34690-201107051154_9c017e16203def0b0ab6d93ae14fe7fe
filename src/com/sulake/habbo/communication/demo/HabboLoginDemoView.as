package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      private var var_145:HabboCommunicationDemo;
      
      private var _window:IWindowContainer;
      
      private var var_165:SSOTicket;
      ...
      {
         this.var_145.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_145 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_145.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_173,HabboWindowStyle.const_47,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_145.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_2064,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_145.habboConfiguration)
            {
               if(this.var_145.habboConfiguration.getKey("use.sso","ICaption") == "true")
               {
                  _loc8_.select();
               }
            }
         ...
         if(this.var_145.habboConfiguration && this.var_145.habboConfiguration.getKey("try.existing.session","ICaption") == "true")
         {
            this.var_1694 = true;
            this._window.visible = false;
            this.windowEventProcessor(WindowEvent.allocate(WindowEvent.const_142,this._window,null,false));
         }
      }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_145.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_145.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_2064,"/");
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_145.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_165.removeEventListener(SSOTicket.const_435,this.onParsedTicketSuccess);
            this.var_165.removeEventListener(SSOTicket.const_351,this.onParsedTicketFailure);
            this.var_165 = null;
         }
         var _loc2_:String = "";
         if(this.var_145.habboConfiguration != null)
         {
            _loc2_ = this.var_145.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_145.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_165 = new SSOTicket(this.var_145.assets,_loc3_,_loc2_,param1);
         this.var_165.addEventListener(SSOTicket.const_435,this.onParsedTicketSuccess);
         this.var_165.addEventListener(SSOTicket.const_351,this.onParsedTicketFailure);
      }
      
      private function onParsedTicketSuccess(param1:Event) : void
      {
         Logger.log("Got ticket: " + this.var_165.ticket);
         Logger.log("Has Facebook: " + this.var_165.isFacebookUser);
         if(this.var_165.isFacebookUser)
         {
            this.var_145.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_145.flashClientUrl = this.var_165.flashClientUrl;
         this.var_145.ssoTicket = this.var_165.ticket;
         this.var_165.removeEventListener(SSOTicket.const_435,this.onParsedTicketSuccess);
         this.var_165.removeEventListener(SSOTicket.const_351,this.onParsedTicketFailure);
         this.var_165 = null;
         dispatchEvent(new Event(const_749));
