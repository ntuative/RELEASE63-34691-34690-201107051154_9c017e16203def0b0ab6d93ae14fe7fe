package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridPartItem implements IAvatarImageListener
   {
      ...
      private var var_779:IFigurePartSet;
      ...
      private const const_2343:int = 13421772;
      
      private const const_2344:int = 16777215;
      
      private const const_387:Array = [2,6,0,4,3,1];
      ...
      public function AvatarEditorGridPartItem(param1:IWindowContainer, param2:IAvatarEditorCategoryModel, param3:IFigurePartSet, param4:Array, param5:Boolean = true)
      {
         ...
         if(param3 == null)
         {
            this.var_753 = new BitmapData(1,1,true,16777215);
         }
         if(param3 != null)
         {
            _loc7_ = param3.parts;
            ...
            {
               this.var_1795 = Math.max(this.var_1795,_loc6_.colorLayerIndex);
            }
         }
         this.var_440 = this.var_78.controller.manager.avatarRenderManager;
      ...
      private function updateThumbVisualization() : void
      {
         ...
         if(_loc1_)
         {
            if(this.var_753 != null && !this.var_1796)
            {
               _loc3_ = this.var_753;
            }
            else
         ...
         if(_loc2_)
         {
            if(this.var_779)
            {
               switch(this.var_779.clubLevel)
               {
                  case HabboClubLevelEnum.const_52:
                     _loc2_.visible = false;
                     break;
                  case HabboClubLevelEnum.const_35:
                     _loc2_.style = HabboIconType.const_870;
                     break;
                  case HabboClubLevelEnum.const_41:
                     _loc2_.style = HabboIconType.const_899;
               }
            }
            else
            {
         ...
         if(this.isSelected)
         {
            this.var_1462.color = this.const_2343;
         }
         else
         {
            this.var_1462.color = this.const_2344;
         }
      ...
      private function analyzePartLayers() : Boolean
      {
         ...
         var _loc1_:IAvatarFigureContainer = this.var_440.createFigureContainer(this.partSet.type + "-" + this.partSet.id);
         Logger.log("QUERYING SET:" + this.partSet.type + "-" + this.partSet.id);
         if(!this.var_440.isFigureReady(_loc1_))
         {
            this.var_440.downloadFigure(_loc1_,this);
            return false;
         }
         var _loc6_:int = 0;
         ...
         for each(_loc2_ in this.partSet.parts)
         {
            if(_loc7_)
            {
               _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_387[_loc6_] + "_" + FigureData.const_173;
               _loc4_ = this.var_440.getAssetByName(_loc3_) as BitmapDataAsset;
            }
            else
            {
               _loc6_ = 0;
               while(!_loc7_ && _loc6_ < this.const_387.length)
               {
                  _loc3_ = "undefined_undefined_" + _loc2_.type + "_" + _loc2_.id + "_" + this.const_387[_loc6_] + "_" + FigureData.const_173;
                  _loc4_ = this.var_440.getAssetByName(_loc3_) as BitmapDataAsset;
      ...
      private function renderThumb() : BitmapData
      {
         ...
         if(this.var_632 == null)
         {
            if(!this.analyzePartLayers())
            {
               if(!this.var_1794)
               {
                  ...
                  this.var_1794 = _loc9_.content as BitmapData;
               }
         ...
         _loc1_ = new BitmapData(this.var_632.width,this.var_632.height,true,16777215);
         var _loc6_:int = 0;
         var _loc7_:Boolean = false;
         ...
         for each(_loc2_ in _loc8_)
         {
            ...
            if(_loc4_)
            {
               ...
               _loc10_ = -1 * _loc4_.offset.x - this.var_632.x;
               _loc11_ = -1 * _loc4_.offset.y - this.var_632.y;
               if(this.var_1796 && _loc2_.colorLayerIndex > 0)
               {
