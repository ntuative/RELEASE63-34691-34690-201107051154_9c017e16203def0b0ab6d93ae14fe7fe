package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_282:IRoomObjectSprite;
      ...
      private var var_1245:Number = 1.0;
      
      private var var_659:BitmapData;
      
      private var var_810:ColorTransform;
      
      private var var_1533:ColorTransform;
      
      private var var_1919:Matrix;
      ...
      {
         super();
         this.var_1244 = new Dictionary();
         this._visualization = param1;
         this.var_810 = new ColorTransform();
         this.var_810.alphaMultiplier = 1;
         this.var_1533 = new ColorTransform();
         this.var_1919 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_114 = this.var_1244[param1];
         this.var_1003 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1920 >= 0)
         {
            this.var_282 = this._visualization.getSprite(this.var_1920);
            if(this.var_282 && this.var_282.asset)
            {
               if(this.var_282.width <= 1 || this.var_282.height <= 1)
               {
                  return;
               }
               if(this.var_100 == null)
               {
                  this.var_100 = this.var_282.asset.clone();
                  if(this.var_810.alphaMultiplier != 1)
                  {
                     this.var_659 = new BitmapData(this.var_100.width,this.var_100.height,true,4278190080);
                  }
               }
               this.var_2355 = -this.var_282.offsetX;
               this.var_2356 = -this.var_282.offsetY;
               this.var_282.asset = this.var_100;
            }
            if(this.var_100)
            {
            ...
            if(this.var_659)
            {
               this.var_659.fillRect(this.var_659.rect,4278190080);
            }
         }
      }
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_114.y * this.var_1245;
         }
         return 0;
      }
      ...
      {
         if(!this.var_114 || !this.var_282)
         {
            return;
         }
         if(this.var_100 && this.var_282.asset != this.var_100)
         {
            this.var_282.asset = this.var_100;
         }
         if(this.var_1003)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_1003 && this.var_114.hasIgnited)
         {
            this.var_1003 = true;
         }
         _loc8_ = this.var_1185 * this.var_1245;
         this.var_114.update();
         if(this.var_1003)
         {
            if(this.var_114.roomObjectSpriteId >= 0)
            ...
            {
               this.var_100.draw(this.var_659,this.var_1919,this.var_810,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_114.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2355 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1245;
               _loc4_ = this.var_2356 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1245;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1533.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_100.draw(_loc7_,this._translationMatrix,this.var_1533,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2628 = parseInt(param1.@size);
         this.var_1920 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1185 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1245 = this.var_2628 / 64;
         this._blend = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this._blend = Math.min(this._blend,1);
         this.var_810.alphaMultiplier = this._blend;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc19_ = _loc23_.@is_emitter == "ICaption" ? false : true;
