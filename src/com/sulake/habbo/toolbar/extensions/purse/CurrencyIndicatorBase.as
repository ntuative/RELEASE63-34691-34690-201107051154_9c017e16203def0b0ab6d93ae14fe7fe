package com.sulake.habbo.toolbar.extensions.purse
{
   ...
   public class CurrencyIndicatorBase implements ICurrencyIndicator
   {
      ...
      private var var_730:IBitmapWrapperWindow;
      ...
      private var var_305:Vector.<BitmapData>;
      
      private var var_2972:uint;
      
      private var var_2974:String = null;
      
      private var var_2136:uint = 0;
      ...
      {
         this.var_305 = new Vector.<BitmapData>();
         super();
         this._view = null;
      ...
      protected function createWindow(param1:String, param2:String) : void
      {
         ...
         if(_loc3_)
         {
            ...
            if(this._view)
            {
               ...
               {
                  this.var_730 = _loc4_[0] as IBitmapWrapperWindow;
      ...
      protected function animateIcon(param1:int) : void
      {
         ...
         if(this.var_730 && this.var_305.length > 0)
         {
            if(this.var_2136 == ANIM_DIRECTION_FORWARD)
            {
               this.var_304 = 0;
            }
            else
            {
               this.var_304 = this.var_305.length - 1;
            }
            this.var_731 = new Timer(this.var_2972,this.var_305.length);
      ...
      protected function setText(param1:String) : void
      {
         if(this._view)
         {
            this._view.findChildByName(this.var_2135).caption = param1;
            this._view.findChildByName(this.var_2134).caption = param1;
         }
      }
      ...
      protected function setTextUnderline(param1:Boolean) : void
      {
         ...
         if(this._view)
         {
            _loc2_ = this._view.findChildByName(this.var_2135) as ITextWindow;
            _loc3_ = this._view.findChildByName(this.var_2134) as ITextWindow;
      ...
      protected function set iconAnimationSequence(param1:Array) : void
      {
         ...
         for each(_loc2_ in param1)
         {
            ...
            if(_loc3_)
            {
               ...
               if(_loc4_)
               {
                  this.var_305.push(_loc4_.clone());
               }
            }
         }
      }
      ...
      private function onAnimationTimer(param1:TimerEvent) : void
      {
         if(this.var_730 && this.var_305.length > 0)
         {
            this.setIconBitmap(this.var_305[this.var_304]);
            if(this.var_2136 == ANIM_DIRECTION_FORWARD)
            {
               ++this.var_304;
               this.var_304 = this.var_304 < this.var_305.length ? int(this.var_304) : int(this.var_305.length - 1);
            }
            else
            {
               --this.var_304;
               this.var_304 = this.var_304 >= 0 ? int(this.var_304) : 0;
            }
         }
      }
      ...
      private function onAnimationTimerComplete(param1:TimerEvent) : void
      {
         ...
         {
            this.setIconBitmap(this.var_305[0]);
         }
      }
      ...
      private function setIconBitmap(param1:BitmapData) : void
      {
         if(this.var_730)
         {
            this.var_730.bitmap = param1.clone();
            this.var_730.width = param1.width;
            this.var_730.height = param1.height;
            this.var_730.invalidate();
         }
      }
      
      private function onContainerMouseOver(param1:WindowMouseEvent) : void
      {
         this._view.color = this.var_2973;
      }
      
      private function onContainerMouseOut(param1:WindowMouseEvent) : void
      {
         this._view.color = this.var_2975;
      }
   }
}
