package com.sulake.habbo.ui.widget.effects
{
   ...
   public class EffectsWidget extends RoomWidgetBase
   {
      ...
      private static const const_1604:int = 320;
      
      private static const const_1605:int = 48;
      ...
      {
         return var_930 as EffectsWidgetHandler;
      }
      ...
      override public function dispose() : void
      {
         ...
         if(this.var_405)
         {
            for each(_loc1_ in this.var_405)
            {
               _loc1_.dispose();
            }
      ...
      public function update() : void
      {
         ...
         {
            _loc1_ = this.var_405.getValue(_loc3_.type) as EffectView;
            if(_loc1_)
            {
               _loc1_.update();
            }
            else
            {
               ...
               this.var_405.add(_loc3_.type,_loc1_);
               this.var_62.addListItem(_loc1_.window);
            }
         }
         _loc4_ = this.var_405.length - 1;
         while(_loc4_ >= 0)
         {
            _loc1_ = this.var_405.getWithIndex(_loc4_);
            if(_loc2_.indexOf(_loc1_.effect) == -1)
            {
               this.var_62.removeListItem(_loc1_.window);
               this.var_405.remove(this.var_405.getKey(_loc4_));
               _loc1_.dispose();
            }
            _loc4_--;
         }
         var _loc5_:int = this.var_62.scrollableRegion.height;
         this.var_62.height = Math.max(Math.min(_loc5_,const_1604),const_1605);
      ...
      {
         this.handler.container.catalog.openCatalogPage(CatalogPageName.const_1195,true);
      }
   }
}
