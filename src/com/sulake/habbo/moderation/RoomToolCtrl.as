package com.sulake.habbo.moderation
{
   ...
   public class RoomToolCtrl implements IDisposable, TrackedWindow
   {
      ...
      private var var_1223:IDropMenuWindow;
      
      private var var_529:ITextFieldWindow;
      
      private var var_615:Boolean = true;
      
      private var var_994:ICheckBoxWindow;
      
      private var var_1509:ICheckBoxWindow;
      
      private var var_1508:ICheckBoxWindow;
      ...
      public function show() : void
      {
         ...
         this.var_36.connection.send(new GetModeratorRoomInfoMessageComposer(this.var_387));
         Logger.log("BEGINNING TO SHOW: " + this.var_387);
      }
      
      public function getType() : int
      {
         return WindowTracker.const_1979;
      }
      ...
      {
         Logger.log("GOT ROOM INFO: " + param1.flatId + ", " + this.var_387);
         if(param1.flatId != this.var_387)
         {
            Logger.log("NOT THE SAME FLAT: " + param1.flatId + ", " + this.var_387);
            return;
         }
         this._data = param1;
         this.populate();
      ...
      {
         this.var_62 = IItemListWindow(this._frame.findChildByName("list_cont"));
         var _loc1_:IWindow = this._frame.findChildByTag("close");
         _loc1_.procedure = this.onClose;
         this.var_529 = ITextFieldWindow(this._frame.findChildByName("message_input"));
         this.var_529.procedure = this.onInputClick;
         this.var_1223 = IDropMenuWindow(this._frame.findChildByName("msgTemplatesSelect"));
         this.prepareMsgSelect(this.var_1223);
         this.var_1223.procedure = this.onSelectTemplate;
         this.var_994 = ICheckBoxWindow(this._frame.findChildByName("kick_check"));
         this.var_1509 = ICheckBoxWindow(this._frame.findChildByName("lock_check"));
         this.var_1508 = ICheckBoxWindow(this._frame.findChildByName("changename_check"));
      ...
      {
         var _loc3_:IWindowContainer = IWindowContainer(this.var_62.getListItemByName(param2));
         ...
         if(!param1.exists)
         {
            this.disposeItemFromList(this.var_62,_loc3_);
            this.disposeItemFromList(this.var_62,this.var_62.getListItemByName("event_spacing"));
            return;
         }
      ...
      private function onChatlog(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_36.windowTracker.show(new ChatlogCtrl(new GetRoomChatlogMessageComposer(0,this._data.flatId),this.var_36,WindowTracker.const_773,this._data.flatId),this._frame,false,false,true);
      }
      ...
      {
         if(this.var_615 || this.var_529.text == "")
         {
            this.var_36.windowManager.alert("Alert","You must input a message to the user",0,this.onAlertClose);
            return;
         }
         var _loc2_:int = this.determineAction(param1,this.var_994.isSelected);
         this.var_36.connection.send(new ModeratorActionMessageComposer(ModeratorActionMessageComposer.const_2031,_loc2_,this.var_529.text,"",""));
         if(this.var_1509.isSelected || this.var_1508.isSelected || this.var_994.isSelected)
         {
            this.var_36.connection.send(new ModerateRoomMessageComposer(this._data.flatId,this.var_1509.isSelected,this.var_1508.isSelected,this.var_994.isSelected));
         }
         this.dispose();
      }
      ...
      private function determineAction(param1:Boolean, param2:Boolean) : int
      {
         if(param2)
         {
            return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1846);
         }
         return !!param1 ? 0 : int(ModeratorActionMessageComposer.const_1821);
      }
      ...
      private function onSelectTemplate(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc3_:String = this.var_36.initMsg.roomMessageTemplates[this.var_1223.selection];
         if(_loc3_ != null)
         {
            this.var_615 = false;
            this.var_529.text = _loc3_;
         }
      }
   }
}
