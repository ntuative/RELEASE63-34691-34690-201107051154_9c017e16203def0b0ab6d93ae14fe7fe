package com.sulake.core.runtime
{
   ...
   public final class CoreComponentContext extends ComponentContext implements ICore
   {
      ...
      private static const const_1652:String = "asset-libraries";
      
      private static const const_1647:String = "library";
      
      private static const const_1650:String = "service-libraries";
      
      private static const const_1651:String = "library";
      
      private static const const_1649:String = "component-libraries";
      
      private static const const_1655:String = "library";
      
      private static const const_1653:String = "error_data";
      
      private static const const_1654:String = "error_cat";
      
      private static const const_1648:String = "error_desc";
       
      
      private var var_415:LibraryLoaderQueue;
      
      private var var_893:IEventDispatcher;
      
      private var var_588:uint;
      
      private var var_414:Function;
      
      private var var_234:Profiler;
      ...
      public function CoreComponentContext(param1:DisplayObjectContainer, param2:uint)
      {
         ...
         {
            this.var_300.push(new Array());
            this.var_718.push(0);
            _loc3_++;
         }
         this._lastUpdateTimeMs = getTimer();
         attachComponent(this,[new IIDCore()]);
         var_431.addEventListener(Event.ENTER_FRAME,this.onEnterFrame);
         switch(param2 & 0)
         {
            ...
            case Core.const_859:
               ...
               this.var_234 = new Profiler(this);
               attachComponent(this.var_234,[new IIDProfiler()]);
               var_431.addChild(new ProfilerViewer(this.var_234));
               break;
            case Core.const_529:
      ...
      {
         return this.var_588 - this.getNumberOfFilesPending();
      }
      ...
      override public function dispose() : void
      {
         ...
         if(!disposed)
         {
            ...
            try
            {
               ...
               {
                  receivers = this.var_300[i] as Array;
            ...
            if(var_431)
            {
               var_431.removeEventListener(Event.ENTER_FRAME,this.onEnterFrame);
               var_431 = null;
            }
      ...
      override public function error(param1:String, param2:Boolean, param3:int = -1, param4:Error = null) : void
      {
         if(param4)
         {
            ErrorReportStorage.setParameter(const_1653,String(param4.getStackTrace()));
         }
         ErrorReportStorage.setParameter(const_1654,String(param3));
         ErrorReportStorage.setParameter(const_1648,param1);
      ...
      public function readConfigDocument(param1:XML, param2:IEventDispatcher = null) : void
      {
         ...
         this.var_893 = loadingEventDelegate;
         if(this.var_415 == null)
         {
            this.var_415 = new LibraryLoaderQueue(_debug);
         }
         try
         {
            node = config.child(const_1652)[0];
            if(node != null)
            {
               list = node.child(const_1647);
         ...
         {
            error("Failed to parse asset libraries from config xml!",true,Core.const_226,e);
         }
         try
         {
            node = config.child(const_1650)[0];
            if(node != null)
            {
               list = node.child(const_1651);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse interfaces from config xml!",true,Core.const_226,e);
         }
         try
         {
            node = config.child(const_1649)[0];
            if(node != null)
            {
               list = node.child(const_1655);
               for each(item in list)
               {
                  url = item.attribute("url").toString();
                  loader = new LibraryLoader(context,true,_debug);
         ...
         {
            error("Failed to parse components from config xml!",true,Core.const_226,e);
         }
         if(!disposed)
         {
      ...
      private function finalizeLoadingEventDelegate() : void
      {
         if(this.var_893)
         {
            this.var_893.dispatchEvent(new Event(Event.COMPLETE));
         }
      }
      ...
      private function updateLoadingProgress(param1:LibraryLoaderEvent = null) : void
      {
         ...
         if(this.var_893)
         {
            ...
            this.var_893.dispatchEvent(new LibraryProgressEvent(_loc2_.url,param1.bytesLoaded,param1.bytesTotal,_loc2_.elapsedTime));
         }
      }
         ...
         if(param1 != null)
         {
            if(param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE || param1.type == LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR)
            {
               ...
               if(!disposed)
               {
                  if(this.var_893)
                  {
                     this.var_893.dispatchEvent(new LibraryProgressEvent(_loc2_.url,this.var_588 - this.var_415.length,this.var_588,_loc2_.elapsedTime));
                  }
               }
            }
         }
      ...
      override public function registerUpdateReceiver(param1:IUpdateReceiver, param2:uint) : void
      {
         ...
         var _loc3_:int = !!this.var_234 ? 0 : this.var_1639 & 0;
         if(_loc3_ == Core.const_529)
         {
            this.var_300[param2].push(new UpdateDelegate(param1,this,param2));
         }
         else
         {
            this.var_300[param2].push(param1);
         }
      }
      ...
      override public function removeUpdateReceiver(param1:IUpdateReceiver) : void
      {
         ...
         var _loc4_:int = !!this.var_234 ? 0 : this.var_1639 & 0;
         var _loc5_:int = 0;
         while(_loc5_ < CoreComponentContext.const_107)
         {
            _loc3_ = this.var_300[_loc5_] as Array;
            if(_loc4_ == Core.const_529)
            {
      ...
      private function simpleFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         {
            this.var_718[priority] = 0;
            receivers = this.var_300[priority];
            index = 0;
            length = receivers.length;
            while(index != length)
            {
      ...
      private function complexFrameUpdateHandler(param1:uint, param2:uint) : void
      {
         ...
         var maxTimePerFrame:uint = 1000 / var_431.stage.frameRate;
         priority = 0;
         while(priority < CoreComponentContext.const_107)
         {
            t = getTimer();
            ...
            {
               if(this.var_718[priority] < priority)
               {
                  ++this.var_718[priority];
                  skip = true;
               }
            }
            if(!skip)
         ...
         while(priority < CoreComponentContext.const_107)
         {
            ...
            while(index != length)
            {
               ...
               else
               {
                  try
                  {
                     this.var_234.update(receiver,msSinceLastUpdate);
                  }
                  catch(e:Error)
                  {
                     error("Error in update receiver \"" + getQualifiedClassName(receiver) + "\": " + e.message,true,e.errorID,e);
      ...
      public function setProfilerMode(param1:Boolean) : void
      {
         ...
         else
         {
            if(this.var_234)
            {
               detachComponent(this.var_234);
               this.var_234.dispose();
               this.var_234 = null;
            }
            switch(this.var_1639 & 0)
            {
               case Core.const_1321:
                  this.var_414 = this.simpleFrameUpdateHandler;
                  break;
